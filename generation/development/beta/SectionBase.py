...
class SectionBase:
    def __init__(self, root, position, title, scrollable, **kwargs):
        # super().__init__(root, position, title, scrollable)
        self.root = root
        self.title = title
    ...
    def initialize_section(self):
        if self.scrollable:
            # Initialize scrollable section with grid
            self._setup_scrollable_frame()
        else:
            # Initialize non-scrollable section with grid
    ...
    def _add_title_label(self):
        title_label = tk.Label(self.frame, text=self.title)
    ...
    def _setup_non_scrollable_frame(self):
        ...
        # Ensure root container is configured for grid usage
        self.root.grid_rowconfigure(self.position[0], weight=1)
        self.root.grid_columnconfigure(self.position[1], weight=1)
